...
def update_and_upload_sp_files(
    run_id: str, sp_chunk_dir: str, sgf_dir: str, sp_chunks: set[str], sgfs: set[str]
    ...
    for sp_chunk in new_sp_chunks:
        # wait for `done` file.
        while sp_chunk.name.split(".")[0] not in map(
            lambda x: x.name.split(".")[0], sp_chunk_dir.glob(DONE_GLOB)
        ):
            time.sleep(0.5)
    ...
    for sgf in new_sgfs:
        # wait for `done` file.
        while sgf.name.split(".")[0] not in map(
            lambda x: x.name.split(".")[0], sgf_dir.glob(DONE_GLOB)
        ):
            time.sleep(0.5)
...
def loop(
    ...
    """
    (_, local_model_cands_dir, _, local_sp_chunk_dir, local_sgf_dir) = (
    ...
    while True:
        ...
        def get_gumbel_params():
            def clamp_k(k):
                # we do not want the final round to have 3 actions to select from.
                num_rounds = int(math.log2(k))
                max_k = 2
            ...
            selected_n = int(
                round(
                    config.min_train_selected_n
                    + c_n * (config.max_train_selected_n - config.min_train_selected_n)
                )
            )
            selected_k = int(
                round(
            ...
            default_k = int(
                round(
                    ...
                    + c_k * (config.max_train_default_k - config.min_train_default_k)
                )
            )
        ...
        while True:
            ...
            sp_chunks, sgfs = update_and_upload_sp_files(
                run_id, local_sp_chunk_dir, local_sgf_dir, sp_chunks, sgfs
            )
        ...
        _, _ = update_and_upload_sp_files(
            run_id, local_sp_chunk_dir, local_sgf_dir, sp_chunks, sgfs
        )
