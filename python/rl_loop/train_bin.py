...
flags.DEFINE_string(
    "local_run_dir", "", "Local path to store models and training chunks."
)
flags.DEFINE_string(
    "model_checkpoint_dir",
    "/tmp/p3achygo_checkpoints",
    "Local path to store intermediate models.",
)
flags.DEFINE_string(
    "val_ds_path", "", "Local path to validation DS. Also used for calibration"
)
...
def main(_):
    ...
    if gcs.get_most_recent_model(FLAGS.run_id) < 0:
        ...
        model_utils.save_trt_and_upload(
            model,
            FLAGS.val_ds_path,
            str(model_dir),
            0,
    ...
    while model_gen < FLAGS.num_generations:
        ...
        chunk_filename = gcs.download_golden_chunk(
            FLAGS.run_id, str(chunk_dir), next_model_gen
        )
        logging.info(f"Training model gen {model_gen}...")
        rl_loop.train.train_one_gen(
            model, model_gen, chunk_filename, val_ds, FLAGS.log_interval, is_gpu
        )
