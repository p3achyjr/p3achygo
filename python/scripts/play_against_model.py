...
flags.DEFINE_string("color", "black", "Color for model to play as (black|white)")
...
def model_move(model, board, last_moves, game_state, color):
    ...
    score_sample = (
        tf.random.categorical(logits=score_logits, num_samples=5).numpy() - 399.5
    )
    ...
    logging.info(f"P3achyGo Predicted Outcome: {tf.nn.softmax(game_outcome).numpy()}")
...
def play(model, color=BLACK):
    ...
    board = GoBoard()
    last_moves = deque(
        [tf.convert_to_tensor(np.zeros((19, 19), dtype=np.float32)) for _ in range(5)]
    )

    komi_state = (
        tf.convert_to_tensor([[0.0]])
        if color == BLACK
        else tf.convert_to_tensor([[0.5]])
    )
    if color == BLACK:
        p0_move = model_move
        p1_move = human_move
