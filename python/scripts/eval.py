...
def run_invocation(cmd: str, gpu_id: int, inv_id: int) -> dict | None:
    ...
    try:
        ...
        if result is None:
            log(
                f"[GPU {gpu_id}] WARNING: Could not parse stats from invocation {inv_id} (exit {proc.returncode})"
            )
        return result
...
def main():
    ...
    parser.add_argument(
        "--cmd", required=True, help="Eval command to run (must include --num_games)"
    )
    parser.add_argument(
        "--num_games", type=int, required=True, help="Total target number of games"
    )
    ...
    print(
        f"Total invocations:    {total_invocations}  ({total_invocations * games_per_cmd} games)"
    )
    ...
    threads = [
        Thread(target=gpu_worker, args=(gpu_id,), daemon=True) for gpu_id in gpu_ids
    ]
    for t in threads:
        t.start()
    for t in threads:
