...
def main(_):
    ...
    model(
        *[tf.convert_to_tensor(x) for x in random_inputs(planes_shape, features_shape)]
    )
    model.summary()
    ...
    def model_fn(board_state: tf.Tensor, game_state: tf.Tensor):
        (
            ...
            q100,
            q200,
        ) = model(board_state, game_state, training=False)
    ...
    tf.io.write_graph(
        graph_or_graph_def=frozen_fn.graph,
