...
def main(_):
    ...
    with tf.device("/cpu:0"):
        ...
        @tf.function
        def model_fn(board_state: tf.Tensor, game_state: tf.Tensor, scores: tf.Tensor):
            (
                pi_logits,
                pi,
                outcome_logits,
    ...
    if FLAGS.val_ds:
        ...
        val_ds = val_ds.map(transforms.expand_v0, num_parallel_calls=tf.data.AUTOTUNE)
        ...
        scores = (
            0.05 * tf.range(-SCORE_RANGE // 2 + 0.5, SCORE_RANGE // 2 + 0.5)
        ).numpy()
