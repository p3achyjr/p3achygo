...
def _download_model_cand(run_id: str, local_models_dir: str, model_dir: str) -> str:
    local_dir = Path(local_models_dir, model_dir)
    gcs_dir = Path(gcs_model_cands_dir(run_id), model_dir)
...
def _download_chunk_file(run_id: str, local_chunk_dir: str, chunk_filename: str) -> str:
    local_chunk_path = Path(local_chunk_dir, chunk_filename)
    gcs_chunk_path = Path(gcs_chunk_dir(run_id), chunk_filename)
...
def upload_chunk_size(run_id: str, local_chunk_dir: str, gen: int):
    _upload_chunk_file(run_id, local_chunk_dir, GOLDEN_CHUNK_SIZE_FORMAT.format(gen))
...
def download_golden_chunk_size(run_id: str, local_chunk_dir: str, gen: int) -> str:
    return _download_chunk_file(
        run_id, local_chunk_dir, GOLDEN_CHUNK_SIZE_FORMAT.format(gen)
    )


def download_golden_chunk(run_id: str, local_chunk_dir: str, gen: int) -> str:
    return _download_chunk_file(
        run_id, local_chunk_dir, GOLDEN_CHUNK_FORMAT.format(gen)
    )
...
def download_model_cand(run_id: str, local_models_dir: str, gen: int) -> str:
    return _download_model_cand(run_id, local_models_dir, MODEL_FORMAT.format(gen))
