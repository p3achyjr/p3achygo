...
class CyclicLRSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(self, min_lr: float, max_lr: float, cycle_len: int):
        ...
        self.half_cycle_len = tf.constant(
            cycle_len // 2 if cycle_len % 2 == 0 else cycle_len // 2 + 1, dtype=tf.int64
        )

        self.lr_delta = tf.constant(max_lr - min_lr) / tf.cast(
            self.half_cycle_len, dtype=tf.float32
        )
...
class CyclicLRDecaySchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(self, min_lr: float, max_lr: float, cycle_len: int, decay_bound=0.95):
        self._main_cycle_len = tf.constant(int(cycle_len * decay_bound), dtype=tf.int64)
        self._half_cycle_len = tf.constant(
            cycle_len // 2 if cycle_len % 2 == 0 else cycle_len // 2 + 1, dtype=tf.int64
        )
        self._decay_len = tf.constant(cycle_len, dtype=tf.int64) - self._main_cycle_len
