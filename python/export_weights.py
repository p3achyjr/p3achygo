...
def fill_conv_layer(conv_layer: tf.keras.layers.Conv2D, group: h5py.Group):
    ...
    kernel = kernel.transpose((3, 2, 0, 1)).copy()  # (k, k, C, C') -> (C', C, k, k)
    group.create_dataset(DatasetTags.KERNEL, data=kernel, dtype=np.float32)
    group.create_dataset(
        DatasetTags.BIAS, data=conv_layer.bias.numpy(), dtype=np.float32
    )
...
def fill_dense_layer(dense_layer: tf.keras.layers.Dense, group: h5py.Group):
    ...
    group.create_dataset(
        DatasetTags.BIAS, data=dense_layer.bias.numpy(), dtype=np.float32
    )
...
def fill_init_conv(model: P3achyGoModel, group: h5py.Group):
    fill_conv_block(model.init_board_conv, group.create_group(BlockTags.CONV_BLOCK))
...
def fill_bottleneck_block(block: BottleneckResidualConvBlock, group: h5py.Group):
...
def fill_broadcast_block(block: BroadcastResidualBlock, group: h5py.Group):
    ...
    def fill_broadcast_layer(
        block: BroadcastResidualBlock.Broadcast, group: h5py.Group
    ):
        fill_dense_layer(
            block.dense, create_ordered_group(group, LayerTags.DENSE, order=0)
        )
        fill_act_layer(
            "relu", create_ordered_group(group, LayerTags.ACTIVATION, order=1)
        )
...
def fill_policy_head(model: P3achyGoModel, group: h5py.Group):
    ...
    fill_conv_layer(policy_head.conv_p, conv_policy_group.create_group(LayerTags.CONV))

    conv_global_group = group.create_group(PolicyHeadTags.CONV_GLOBAL)
    fill_conv_layer(policy_head.conv_g, conv_global_group.create_group(LayerTags.CONV))

    fill_global_pool_bias(
        policy_head.gpool, group.create_group(BlockTags.GLOBAL_POOL_BIAS)
    )
    fill_bn_layer(policy_head.batch_norm, group.create_group(LayerTags.BATCH_NORM))

    conv_moves_group = group.create_group(PolicyHeadTags.CONV_MOVES)
    fill_conv_layer(
        policy_head.output_moves, conv_moves_group.create_group(LayerTags.CONV)
    )

    dense_pass_group = group.create_group(PolicyHeadTags.DENSE_PASS)
    fill_dense_layer(
        policy_head.output_pass, dense_pass_group.create_group(LayerTags.DENSE)
    )
...
def fill_value_head(model: P3achyGoModel, group: h5py.Group):
    ...
    # prep
    fill_conv_layer(value_head.conv, conv_value_group.create_group(LayerTags.CONV))
    ...
    fill_dense_layer(
        value_head.outcome_q_embed,
        dense_outcome_pre_group.create_group(LayerTags.DENSE),
    )
    dense_outcome_group = group.create_group(ValueHeadTags.DENSE_OUTCOME)
    fill_dense_layer(
        value_head.outcome_q_output, dense_outcome_group.create_group(LayerTags.DENSE)
    )

    # ownership
    ownership_group = group.create_group(ValueHeadTags.OWNERSHIP)
    fill_conv_layer(
        value_head.conv_ownership, ownership_group.create_group(LayerTags.CONV)
    )

    # gamma
    dense_gamma_pre_group = group.create_group(ValueHeadTags.DENSE_GAMMA_PRE)
    fill_dense_layer(
        value_head.gamma_pre, dense_gamma_pre_group.create_group(LayerTags.DENSE)
    )
    dense_gamma_group = group.create_group(ValueHeadTags.DENSE_GAMMA)
    fill_dense_layer(
        value_head.gamma_output, dense_gamma_group.create_group(LayerTags.DENSE)
    )
    ...
    fill_dense_layer(
        value_head.score_pre, dense_score_pre_group.create_group(LayerTags.DENSE)
    )
    dense_score_group = group.create_group(ValueHeadTags.DENSE_SCORES)
    fill_dense_layer(
        value_head.score_output, dense_score_group.create_group(LayerTags.DENSE)
    )
