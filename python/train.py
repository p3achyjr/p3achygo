...
def train_step_v1(
    ...
    with tf.GradientTape() as g:
        ...
        (
            ...
            w_q_err=w_q_err,
            w_q_score=w_q_score,
            w_q_score_err=w_q_score_err,
            w_pi_soft=w_pi_soft,
            w_pi_optimistic=w_pi_optimistic,
        )
...
def train(
    ...
    if model.version == 0:
        ...
        train_fn = functools.partial(
            ...
            coeffs.w_q_err,
            coeffs.w_q_score,
            coeffs.w_q_score_err,
            coeffs.w_pi_soft,
            coeffs.w_pi_optimistic,
        )
                ...
                with summary_writer.as_default():
                    ...
                    for grad, var in zip(gradients, model.trainable_variables):
                        tf.summary.histogram(
                            var.name + "/gradient", grad, step=batch_num
                        )
...
def log_train(
        ...
        for i in range(len(own_pred)):
            s.append(
                (
                    ...
                    + " "
                    + " ".join(
                        [char_at(own_pred, i, j) for j in range(len(own_pred[0]))]
                    )
                )
            )
    ...
    top_score_indices = (
        tf.math.top_k(score_logits[0], k=5).indices - SCORE_RANGE_MIDPOINT
    )
    top_score_values = tf.math.top_k(score_logits[0], k=5).values
    actual_policy = pi[0] if len(pi.shape) == 1 else tf.math.argmax(pi[0])
    actual_policy_aux = (
        pi_aux[0] if len(pi_aux.shape) == 1 else tf.math.argmax(pi_aux[0])
    )
    ...
    print(f"===== POLICY INFO =====")
    print(
        f"Predicted Top 5 Moves:",
        f"{[move(mv) for mv in top_policy_indices]}",
    ...
    print(f"===== Q/Z INFO =====")
    print(
        f"Predicted Outcome: {tf.nn.softmax(outcome_logits[0])},",
...
def val(
    ...
    for (
        ...
        q100,
        q200,
        *_extra,  # Ignore extra v1 labels if present
    ) in val_ds:
        (
            pi_logits,
            pi_logits_aux,
        ...
        true_move = (
            ...
            else tf.math.argmax(policy, axis=1, output_type=tf.int32)
        )
        predicted_move = tf.math.argmax(pi_logits, axis=1, output_type=tf.int32)
        predicted_outcome = tf.math.argmax(outcome_logits, axis=1, output_type=tf.int32)

        correct_move = tf.cast(tf.equal(true_move, predicted_move), dtype=tf.int32)
        correct_outcome = tf.cast(
            tf.equal(
                tf.where(
                    score >= 0,
                    tf.ones_like(predicted_outcome, dtype=tf.int32),
...
def log_val(batch_num: int, losses: LossTracker, metrics: ValMetrics):
    ...
    print(
        "Correct Moves: ", metrics.correct_moves, ", Total Moves: ", metrics.num_moves
    )
    print(
        "Correct Outcomes: ",
    ...
    log("Prediction Moves Percentage", float(metrics.correct_moves) / metrics.num_moves)
    log(
        "Prediction Outcome Percentage",
