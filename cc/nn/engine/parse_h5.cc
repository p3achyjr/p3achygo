...
static constexpr char kBoardLen[] = "board_len";
...
Layer ParseConvLayer(H5::Group conv_group) {
  ...
  CHECK(layer.properties[Layer::kKernel].weights->shape[0] ==
...
std::unique_ptr<Model> ParseModel(H5::Group model_group) {
  ...
  model->board_len =
      ReadIntAttr(model_group.openAttribute(tags::metadata::kBoardLen));
