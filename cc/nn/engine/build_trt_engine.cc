...
class Logger : public nv::ILogger {
 public:
  ...
  void log(Severity severity, nv::AsciiChar const* msg) noexcept override {
...
void PrintDims(nv::Dims dims) {
  std::cerr << "Dims. Size: " << dims.nbDims << ", Kernel: (";
  for (int i = 0; i < std::max(1, dims.nbDims); ++i) {
    std::cerr << dims.d[i] << ", ";
  }
  std::cerr << ")\n";
}
...
nv::ITensor* BuildConvLayer(nv::INetworkDefinition* network,
  ...
  logger().log(nv::ILogger::Severity::kINFO,
               "Building Conv Layer: " + layer.name);
...
nv::ITensor* BuildDenseLayer(nv::INetworkDefinition* network,
  ...
  // Add Matmul.
  nv::ITensor* kernel_tensor =
      network->addConstant(nv::Dims2(in_dim, out_dim), kernel_weights)
          ->getOutput(0);
  nv::IMatrixMultiplyLayer* matmul_layer =
      AddMatmul(network, input_tensor, kernel_tensor, layer.name + ":kernel");
  nv::ITensor* matmul_output = matmul_layer->getOutput(0);

  // Add Bias.
  nv::Dims2 bias_dims(1, out_dim);
  nv::ITensor* bias_tensor =
      network->addConstant(bias_dims, bias_weights)->getOutput(0);
...
nv::ITensor* BuildBatchNormLayer(nv::INetworkDefinition* network,
  ...
  nv::ITensor* moving_mean_tensor =
      network->addConstant(bn_dims, moving_mean_weights)->getOutput(0);
  ...
  nv::ITensor* moving_variance_tensor =
      network->addConstant(bn_dims, moving_variance_weights)->getOutput(0);

  // Beta Weights.
  nv::Weights beta_weights{nv::DataType::kFLOAT, beta_data.data(),
                           static_cast<int64_t>(beta_data.size())};
  nv::ITensor* beta_tensor =
      network->addConstant(bn_dims, beta_weights)->getOutput(0);

  // Epsilon Weights.
  nv::Weights eps_weights{nv::DataType::kFLOAT, epsilon, 1};
  nv::ITensor* eps_tensor =
      network->addConstant(nv::Dims4(1, 1, 1, 1), eps_weights)->getOutput(0);
  ...
  nv::IUnaryLayer* sqrt_layer =
      network->addUnary(*var_eps_tensor, nv::UnaryOperation::kSQRT);
...
PolicyHeadOutput BuildPolicyHead(nv::INetworkDefinition* network,
  ...
  auto validate = [&]() {
    ...
    CHECK(conv_moves_shape.size() == 4 && conv_moves_shape[3] == 2);
  ...
  nv::IConstantLayer* const3 = network->addConstant(
      nv::Dims2(1, 1), nv::Weights{nv::DataType::kFLOAT, &kPassOffset, 1});
...
ValueHeadOutput BuildValueHead(nv::INetworkDefinition* network,
  ...
  auto validate = [&]() {
    ...
    CHECK(value_head.conv_value.properties[Layer::kKernel].weights->shape[3] ==
          num_head_channels);
  ...
  // Constant Score Tensor (800, 1).
  nv::Dims2 score_dims(constants::kNumScoreLogits, 1);
  ...
  nv::IConstantLayer* scores_layer =
      network->addConstant(score_dims, score_weights);
  nv::ITensor* scores = scores_layer->getOutput(0);

  // Reshape Batch Dim, (1, 800, 1) -> (N, 800, 1)
  // nv::ITensor* scores_runtime_shape =
  //     StaticShapeWithBatch(network, gpool_tiled, kScoreDimsPartial, 2);
  // nv::IShuffleLayer* scores_set_batch_dim =
  //     AddShuffle(network, scores_layer->getOutput(0),
  //                value_head.name + ":scores_set_batch_dim");
  // scores_set_batch_dim->setInput(1, *scores_runtime_shape);
  // nv::ITensor* scores = scores_set_batch_dim->getOutput(0);
